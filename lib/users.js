...
/**
 A function to logout a user for a realm
 @param {string} realmName - The name of the realm(not the realmID) - ex: master,
 ...
 keycloakAdminClient(settings)
 .then((client) => {
      client.users.logout(realmName, userId)
        .then(() => {
          console.log('success')
      })
    })
 */
function logout (c) {
  return function logout (realmName, userId) {
    return new Promise((resolve, reject) => {
      if (!userId) {
      ...
      const req = {
        url: `${c.baseUrl}/admin/realms/${realmName}/users/${userId}/logout`,
        auth: {
